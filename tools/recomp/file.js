...
var File = {};
{
	...
	function create(fstream, ftype, fseek=0){
		return ((function(fstream, ftype, fseek){
			var self = {};
			var stream = fstream;
			...
			var dbuff = new Float64Array(buff.buffer);
			...
			self.readbiu32 = function(){
				if(self.mode==File.LITTLE_ENDIAN){
					return (BigInt(self.read8())<<0n) | (BigInt(self.read8())<<8n) | (BigInt(self.read8())<<16n) | (BigInt(self.read8())<<24n);
				}
				else{
					return (BigInt(self.read8())<<24n) | (BigInt(self.read8())<<16n) | (BigInt(self.read8())<<8n) | (BigInt(self.read8())<<0n);
				}
			}
			...
			self.read64 = function(){
				if(self.mode==File.LITTLE_ENDIAN){
					return (BigInt(self.read8())<<0n) | (BigInt(self.read8())<<8n) | (BigInt(self.read8())<<16n) | (BigInt(self.read8())<<24n) | (BigInt(self.read8())<<32n) | (BigInt(self.read8())<<40n) | (BigInt(self.read8())<<48n) | (BigInt(self.read8())<<56n);
				}
				else{
					return (BigInt(self.read8())<<56n) | (BigInt(self.read8())<<48n) | (BigInt(self.read8())<<40n) | (BigInt(self.read8())<<32n) | (BigInt(self.read8())<<24n) | (BigInt(self.read8())<<16n) | (BigInt(self.read8())<<8n) | (BigInt(self.read8())<<0n);
				}
			}
			
			self.write64 = function(v){
				if(self.mode==File.LITTLE_ENDIAN){
					self.put((BigInt(v)>>>0n) & 0xFFn);
					self.put((BigInt(v)>>>8n) & 0xFFn);
					self.put((BigInt(v)>>>16n) & 0xFFn);
					self.put((BigInt(v)>>>24n) & 0xFFn);
					self.put((BigInt(v)>>>32n) & 0xFFn);
					self.put((BigInt(v)>>>40n) & 0xFFn);
					self.put((BigInt(v)>>>48n) & 0xFFn);
					self.put((BigInt(v)>>>56n) & 0xFFn);
				}
				else{
					self.put((BigInt(v)>>>56n) & 0xFFn);
					self.put((BigInt(v)>>>48n) & 0xFFn);
			...
			self.saveAtPath = function(fs, path){
				var nstream = stream.slice(0, stream.length);
				fs.writeFileSync(path, new Uint8Array(nstream));
			}
	...
	//const fs = require('fs');
	self.loadAtPath = function(fs, path, binary) {
		var data = null;
		if (binary) data = fs.readFileSync(path);
		else data = fs.readFileSync(path, "utf8");
		var stream = data;
		/*for(var i=0; i<data.length; i++){
			if (binary) stream.push(data[i]&0xFF);
			else stream.push((data.charCodeAt(i))&0xFF);
		}*/
		var file = create(stream, self.READ_WRITE, 0);
		file.name = path.split(/\\\/\:/g)[path.split(/\\\/\:/g).length-1];
		return file;
	}
